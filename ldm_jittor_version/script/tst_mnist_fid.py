...
def save_images_to_folder(images, folder_path, is_real=False):
    ...
    if is_real:
        ...
        for i, img in enumerate(images):
            ...
            # 保存为PNG文件
            img_np = (img_np * 255).astype(np.uint8)
            cv2.imwrite(os.path.join(folder_path, f"fake_{i:05d}.png"), img_np)
...
if __name__ == "__main__":
    ...
    # 加载训练好的模型权重
    model_path = "./result/ddpm/mnist/ddpm_mnist_final.pth"
    ...
    for batch_idx in tqdm(range(num_batches), desc="生成图像批次"):
        ...
        # 保存当前批次的生成图像
        save_images_to_folder(generated_images_batch.numpy(), fake_path)

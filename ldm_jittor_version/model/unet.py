...
class AttentionBlock(nn.Module):
    ...
    def execute(self, x):
        b, c, h, w = x.shape
        q, k, v = jt.split(self.to_qkv(self.norm(x)), self.in_channels, dim=1)

        q = q.permute(0, 2, 3, 1).view(b, h * w, c)
        k = k.view(b, c, h * w)
        v = v.permute(0, 2, 3, 1).view(b, h * w, c)

        dot_products = jt.bmm(q, k) * (c ** (-0.5))
        assert dot_products.shape == (b, h * w, h * w)

        attention = nn.softmax(dot_products, dim=-1)
        out = jt.bmm(attention, v)
...
class ResidualBlock(nn.Module):
    ...
    def execute(self, x, time_emb=None, y=None):
...
class UNet(nn.Module):
    ...
    def execute(self, x, time=None, y=None):
        ip = self.initial_pad
        if ip != 0:
            x = nn.pad(x, (ip,) * 4)
